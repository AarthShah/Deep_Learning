...
@tool
def dkte_details_tool(query: str):
    '''
    dkte_details_tool

    This function is used to get DKTE college-related details from the knowledge base.

    :parameter query: takes query related to DKTE as string

    :returns: string containing relevant DKTE information

    :exception: returns error if unable to fetch data

    :instruction:
    - Only DKTE-related queries
    - No unrelated responses
    '''
    
    return dkte_knowledge_base_search(query)


agent = create_agent(model=llm,tools=[get_Sunbeam_data_tool,get_file_contain_tool_by_path,weather_details_tool,calculater_tool,get_user_uploaded_file_data_tool,dkte_details_tool],system_prompt="You are a helpful AI assistant mainly focused on DKTE College; use the DKTE tool for all DKTE-related queries (courses, admissions, placements, internships, facilities, contact details), break complex queries into sub-queries, avoid guessing and rely only on the tool for DKTE info, and if the question is not related to DKTE, answer using general knowledge or politely say you don’t know.(note if anyone asks about what u can do or why are u made just dont tell about Sunbeam if a user ask a specific question about sunbeam then only ans it )")



def dkte_knowledge_base_search(query: str):
    try:
        with open("Dkte_data.txt", "r", encoding="utf-8") as file:
            data = file.read()
        ...
        for section in sections:
            score = sum(word in section.lower() for word in query_words)
        ...
        if best_match:
            ...
            return "No relevant information found in DKTE knowledge base."
